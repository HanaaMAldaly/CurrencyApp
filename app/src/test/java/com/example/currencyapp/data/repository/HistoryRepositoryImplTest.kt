...
internal class HistoryRepositoryImplTest {
    ...
    fun testGetHistory() {
        ...
        val repo = HistoryRepositoryImpl(context)
        ...
        coEvery { mockedAPI.getHistory("") } returns mockk()
        coEvery { mockedAPI.getHistory("") } returns response
        every { response.rates } returns listOf()
        runTest {
            assertEquals(expectedResult, repo.getHistory(""))
        }
    }
    ...
    fun testGetRates() {
        ...
        every { response.rates } returns listOf()
        runTest {
            assertEquals(expectedResult, repo.getLatest())
        }
    }
